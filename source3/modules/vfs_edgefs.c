...
static int
vfs_edgefs_smb_fname2inode_full(const void *ctx,
    struct vfs_handle_struct *handle, const struct smb_filename *smb_fname,
    inode_t *inode, char **full_path, char **vfs_path)
{
	char *name;
	char *path;
	int i, l, ret = 0;
	...
	if (name[0] == '/') {
		path = talloc_strdup(ctx, name); // TODO associate to VFS handle
	} else {
		path = talloc_asprintf(ctx, "%s/%s", HANDLE_CWD(handle), name);
	}
	if (path == NULL)
		return (errno);

	if ((full_path == NULL) && (vfs_path != NULL)) {
		DBG_ERR("%d: vfs_path is substring of full_path."
		    "vfs_path should be used together with full_path.",
		    __LINE__);
		ret = EINVAL;
		goto fail;
	}
...
vfs_edgefs_smb_fname2inode(struct vfs_handle_struct *handle,
    const struct smb_filename *smb_fname, inode_t *inode)
{

	return (vfs_edgefs_smb_fname2inode_full(NULL, handle, smb_fname, inode,
		    NULL, NULL));
}
/**
    ...
    const char *user)
{
	...
	HANDLE_CWD(handle) = talloc_strdup(handle->conn, "/");
		...
		const char *mask, uint32_t attributes)
{
	...
	dir = TALLOC_ZERO(NULL, sizeof(struct vfs_edgefs_dir));
	if (dir == NULL) {
		errno = ENOMEM;
		goto fail;
	}
...
edgefs_readdir_cb4(inode_t parent, fsio_dir_entry *de, uint64_t p, void *ptr)
{
	...
	for (i = 0; i < p; i++) {
		n = talloc_strdup(dir->dirents, de[i].name);
		if (n == NULL)
			return (errno);
    ...
    SMB_STRUCT_STAT *sbuf)
{
	...
	if (dir->dirents_cnt == 0 || /* If not fetched yet. */
		...
		n = talloc_strdup(dir, (dir->dirents_cnt > 0) ?
		    dir->dirents[dir->dirents_cnt - 1].name : "");
		if (n == NULL)
			return (NULL);

		/* Free names allocated by strdup.  */
		talloc_free(dir->dirents);
		dir->dirents = talloc_array(dir, fsio_dir_entry,
		    CCOW_FSIO_DIRENT_PREFETCH);
		if (dir->dirents == NULL) {
			return (NULL);
		}
	...
	dir->dirent = talloc_size(dir, sizeof(struct dirent) +
    ...
    struct smb_filename *smb_fname, files_struct *fsp, int flags, mode_t mode)
{
	...
	if (flags & O_DIRECTORY) {
		...
		fname = talloc_asprintf(NULL, "%s/%s", HANDLE_CWD(handle),
    ...
    void *data, size_t n, off_t offset)
{
	...
	DBG_INFO("%d: ccow_fsio_read(file, off: %lu, n: %lu, data, read: %lu, eof: %d) return %lu\n",
	    __LINE__, offset, n, read_amount, eof, ret);
	if (ret == 0) {
		ret = read_amount;
	} else {
		errno = ret;
...
vfs_edgefs_pread_do(void *private_data)
{
	...
	do {
		ret = ccow_fsio_get_size(state->file, &read_amount);
		if (ret != 0) {
			state->ret = -1;
			errno = ret;
		}
		if (state->offset >= read_amount) {
			DBG_INFO("%d: offset: %lu <> size %lu\n", __LINE__, state->offset, read_amount);
			state->ret = 0;
			errno = 0;
			break;
		}
		...
		DBG_INFO("%d: ccow_fsio_read(file, off: %lu, n: %lu, data, read: %lu, eof: %d) return %d\n",
		    __LINE__, state->offset, state->count, read_amount, eof, ret);
		if (ret == 0) {
...
vfs_edgefs_pwrite_do(void *private_data)
{
	...
	do {
		/* Allocate ccowfsio own buffer. It will be freed in CCOW. */
		char *buf = (char *)ccow_fsio_mem_alloc(state->count);
		if (buf == NULL) {
			DBG_ERR("Can't allocate memory for write buffer.\n");
			state->ret = -1;
			errno = ENOMEM;
		}
		/* Freed by CCOW FSIO. */
		memcpy(buf, state->buf, state->count);
		ret = ccow_fsio_write(state->file, state->offset, state->count,
		    (void *)buf, &wrote);
		if (ret == 0) {
			state->ret = wrote;
		} else {
			DBG_ERR("ccow_fsio_write(file, off=%lu, sz=%lu, buf, "
			    "wrote=%lu) return %d.\n", state->offset,
			    state->count, wrote, ret);
			state->ret = -1;
			errno = ret;
			ccow_fsio_mem_free(buf);
		}
	} while ((state->ret == -1) && (errno == EINTR));
    ...
    const SMB_STRUCT_STAT *sbuf)
{
	...
	ret = sbuf->st_ex_blksize * sbuf->st_ex_blocks;
    ...
    const struct smb_filename *smb_fname, uid_t uid, gid_t gid)
{
	...
	return vfs_edgefs_chown(handle, smb_fname, uid, gid); //XXX Resolve links here
}
    ...
    const struct smb_filename *smb_fname)
{
	...
	if (smb_fname->base_name[0] == '/') {
		newpath = talloc_strdup(handle->conn, smb_fname->base_name);
		if (newpath == NULL)
			return (errno);
	} else {
		newpath = talloc_asprintf(handle->conn, "%s/%s",
		    handle->conn->cwd_fname->base_name, smb_fname->base_name);
		if (newpath == NULL)
			return (errno);
		if (normalize_path(newpath) != 0)
			return (EINVAL);
	}
	talloc_free(HANDLE_CWD(handle));
    ...
    const struct smb_filename *smb_fname)
{
	...
	char *result = NULL;
	int ret;
	DBG_INFO("%d: %s /////// %s\n", __LINE__, HANDLE_CWD(handle), smb_fname->base_name);
	ret = vfs_edgefs_smb_fname2inode_full(NULL, handle, smb_fname, NULL,
		    &result, NULL);
	if (ret != 0) {
		return (NULL);
	}
    ...
    off_t offset, off_t count, int type)
{
	...
	if (op == F_GETLK)
		...
		ret = ccow_fsio_lock(HANDLE_CI(handle), file->ino,
		    f2plock_type(type), offset, count);
    ...
    const struct smb_filename *smb_fname, char *buf, size_t bufsiz)
{
	...
	if ((strlen(target) + 1) > bufsiz) {
		errno = EINVAL;
		ret = -1;
	} else {
		memcpy(buf, target, (strlen(target) + 1));
	}
	ccow_fsio_mem_free(target);
fail:
    ...
    const struct smb_filename *new_smb_fname)
{
	...
	tmp.base_name = newparent_path;
	ret = vfs_edgefs_smb_fname2inode(handle, &tmp, &newparent);
	if (ret != 0) {
		DBG_ERR("Failed to fetch edgefs inode (ERROR: %s)\n",
		    strerror(ret));
		goto fail;
	...
	ret = vfs_edgefs_smb_fname2inode(handle, old_smb_fname, &oldinode);
	if (ret != 0) {
		DBG_ERR("Failed to fetch edgefs inode (ERROR: %s)\n",
		    strerror(ret));
		goto fail;
	...
	ret = ccow_fsio_link(HANDLE_CI(handle), newparent,
	    new_smb_fname->base_name, oldinode);
	if (ret != 0) {
		errno = ret;
		ret = -1;
	}
